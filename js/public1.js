...
window.addEventListener("load", function () {
  if (window.localStorage.getItem("user") == "true") {
    user.innerHTML = `<a href="##">${window.localStorage.getItem("tel")}</a>`;
  }
});
...
function priceCalculations() {
  ...
  for (let i = 0; i < textCount.length; i++) {
    ...
    totalPrice.innerHTML =
      (totalPrice.innerHTML - 0 + (shoppingPrice[i].innerHTML - 0)).toFixed(2) -
      0;
...
for (let i = 0; i < textCount.length; i++) {
  ...
  textCount[i].addEventListener("input", function () {
    ...
    if (isNaN(this.value) || this.value.indexOf(".") != -1 || this.value < 1) {
...
function remove(this_) {
  if (this_.nextElementSibling.value == 0) {
    ...
    if (bool) {
      // 删除前先获取下标
      var index =
        this_.parentNode.parentNode.parentNode.parentNode.getAttribute("index");
      // 把下标放入localStorage中
      window.localStorage.setItem("lis" + index, index);
